...
import { useContractRead, useContractWrite, useAccount, useDisconnect } from 'wagmi';
...
export function VideoAuctionSheet({ isOpen, onClose }: VideoAuctionSheetProps) {
  ...
  useEffect(() => {
    if (isConnected && !wasConnected) {
      ...
      // Reopen the sheet after successful connection
      setTimeout(() => {
  ...
  const handleChainClick = async (openChainModal: () => void) => {
    setIsWalletModalOpen(true);
    setShouldShowSheet(false);
    openChainModal();
  };
    ...
    >
      <SheetContent side="right" className="w-full sm:max-w-[900px] bg-[#1752F0] border-none p-6 overflow-y-auto">
        ...
        <style jsx global>{`
          ...
          /* Ensure RainbowKit modal appears above Sheet */
          [data-rk] {
            --rk-overlay-z-index: 2147483646;
            --rk-modal-z-index: 2147483647;
          }

          /* Custom styling for RainbowKit button */
          [data-rk] button {
            font-family: monospace !important;
          }

          /* Override RainbowKit default styles */
          [data-rk] .connect-wallet-btn {
          ...
          [data-rk] .connect-wallet-btn:hover {
